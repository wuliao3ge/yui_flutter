...
// success icon
const Widget _successIcon = Icon(YuiIcons.wancheng, color: Colors.white, size: 49.0);
// fail icon
const Widget _failIcon = Icon(YuiIcons.wancheng, color: Colors.white, size: 49.0);
// 对齐方式
...
class YuiToast {
  ...
  static _info info(BuildContext context) {
    return (message, { duration, align, distance = 100.0 }) async {
      ...
      final remove = createOverlayEntry(
        ...
        child: InfoWidget(
          messageWidget,
          align: _weToastAlign[align == null ? YuiConfig.instance.toastInfoAlign.index : align.index],
          distance: distance
        )
      );
      // 自动关闭
      await Future.delayed(Duration(milliseconds: duration == null ? YuiConfig.instance.toastInfoDuration : duration));
      remove();
    };
  }
  ...
  static _loading loading(BuildContext context) {
    _close show({ message, duration, mask = true, icon }) {
      final int toastLoadingDuration = YuiConfig.instance.toastLoadingDuration;

      return YuiToast.toast(context)(
  ...
  static _success success(BuildContext context) {
    return ({ message, duration, mask = true, icon = _successIcon, onClose }) {
      final int toastSuccessDuration = YuiConfig.instance.toastSuccessDuration;
      YuiToast.toast(context)(
        icon: icon,
        mask: mask,
        message: message,
  ...
  static _fail fail(BuildContext context) {
    return ({ message, duration, mask = true, icon = _failIcon, onClose }) {
      final int toastFailDuration = YuiConfig.instance.toastFailDuration;
      YuiToast.toast(context)(
        icon: icon,
        mask: mask,
        message: message,
